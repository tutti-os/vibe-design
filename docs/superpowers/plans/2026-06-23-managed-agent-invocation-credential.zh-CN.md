...
- 请求目标是本机 loopback host：`localhost`、`127.0.0.1` 或 `[::1]`
- scheme 允许 `http` 或 `https`，但不允许非 loopback host
- 如果未来需要托管 production host，必须先由 TSH 引入显式 trusted origin registry，再扩展这里的 allowlist
...
- `apps/tsh-desktop/src/app/main/websiteWindow/managedAgentInvocationCredentialHeader.ts`
  - 新增文件
  - 负责 loopback host、scheme、method、path 匹配
