...
en_description: |
  Parse a structured design brief written in I-Lang protocol format or natural language into a concrete design spec, reducing ambiguity by making palette, typography, layout, mood, density, and constraints explicit.
zh_description: |
  将 I-Lang 协议格式或自然语言设计简报解析为具体设计规范，明确调色板、字体、布局、氛围、密度和约束，减少“专业一点”等模糊描述带来的歧义。
triggers:
  - "design brief"
...
The values listed below form a closed vocabulary. Only values in this table have concrete token mappings in Section 2.1. If the user provides a value not listed here, the agent must ask for clarification through the `AskUserQuestion` interaction contract rather than guessing.

Clarification questions must use structured `AskUserQuestion` / `ask_user_question` / `request_user_input` events when the provider can emit them. If the provider cannot emit a structured tool-call event, use one inline `<question-form>` block as the non-MCP fallback. Do not render clarification questions as prose, markdown lists, tables, fake tool JSON, or code blocks.
...
Symbolic values not in this table are not valid. If the user provides an unrecognized value (e.g., `palette=ocean_blue`), the agent must ask through `AskUserQuestion`: "I don't recognize `palette=ocean_blue`. Did you mean `navy_and_white`, `monochrome_dark`, `light_clean`, or `earth_tones`?"
...
Produce a DESIGN.md following the standard 9-section convention. All color hex values, font stacks, and spacing values must come from the resolved tokens in Section 2.1 — do not invent values outside the resolution table.
