...
Prototype Design 是一个 AI 辅助的设计原型工作台。从一句需求开始，让生成结果落在真实的设计系统约束里，在交互式画布上预览实际效果，对需要改动的地方精确标注，再把视觉反馈直接交给本地编码 Agent（Codex 或 Claude Code）继续处理。项目、会话、文件和批注都会被保留——原型在多轮迭代中不断变好，而不是每次都从零开始。
