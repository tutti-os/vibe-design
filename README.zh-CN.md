...
Prototype Design 是一个 AI 辅助的设计原型工作台。从一句需求开始，让生成结果落在真实的设计系统约束里，在交互式画布上预览实际效果，对需要改动的地方精确标注，再把视觉反馈直接交给 Tutti 当前暴露的 Agent Target 继续处理。项目、会话、文件和批注都会被保留——原型在多轮迭代中不断变好，而不是每次都从零开始。

灵感来自 Open Design，Prototype Design 让创作流程保持开放、可检查，并始终落在真实项目产物上。

## 为什么选择 Prototype Design
...
- **跑在当前可用 Agent 上** —— 读取 Tutti Agent 列表并使用选中的精确 Agent Target。
...
项目编辑器围绕会话工作区构建。你可以选择当前 Agent Target、切换智能体与模型、引用项目文件、附加视觉评论，让每一次生成和修改都保留正确的上下文。
...
服务端读取当前 Tutti Agent 列表，并按每个会话选中的 exact Agent Target 执行，同时在 UI 中提示可用性问题。列表默认值只用于新会话的选择元数据；Provider id 只作为 runtime 元数据，不再代表用户可见的 Agent 身份。
...
1. 在首页输入项目名称和需求描述。
2. 选择一个官方设计系统作为生成约束。
3. 进入项目编辑器，使用当前可用的 Agent Target 生成原型文件。
...
- pnpm 10.x。
- 当前 Tutti Agent 列表中至少有一个可用 Agent Target。
...
|-- skills/          # 内置 Prototype Design skills
...
Prototype Design 使用 [Apache License 2.0](./LICENSE) 开源。
