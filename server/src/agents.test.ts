...
describe('createAgentRegistry', () => {
  it('lists and looks up ACP provider definitions', () => {
    ...
    expect(registry.listAgentDefs().map((agent) => agent.id)).toEqual(['claude-code', 'codex']);
  ...
  it('throws on duplicate ids', () => {
    expect(() => createAgentRegistry([claudeAgentDef, { ...claudeAgentDef }])).toThrow(/Duplicate agent definition id: claude-code/);
  });
  ...
  it('reports connection test results from probe injection', async () => {
    const registry = createAgentRegistry([claudeAgentDef], async (agent) => ({
      ok: agent.id === 'claude-code',
      latencyMs: 2,
    }));
    await expect(registry.testAgentConnection('claude-code')).resolves.toEqual({ ok: true, latencyMs: 2 });
  ...
  it('reports a failed connection result when no probe is configured', async () => {
    ...
    await expect(registry.testAgentConnection('claude-code')).resolves.toEqual({
      ok: false,
  ...
  it('protects registry state from source descriptor mutation after creation', () => {
    ...
    expect(registry.listAgentDefs().map((agent) => agent.id)).toEqual(['claude-code']);
    expect(registry.getAgentDef('claude-code')?.label).toBe('Claude Code');
    expect(registry.isKnownModel('claude-code', 'mutated-model')).toBe(false);
  });
  ...
  it('protects registry state from returned agent definition mutation', () => {
    ...
    expect(registry.listAgentDefs().map((agent) => agent.id)).toEqual(['claude-code']);
    expect(registry.isKnownModel('claude-code', 'returned-model')).toBe(false);
  });

  it('protects registry state from returned lookup mutation', () => {
    const registry = createAgentRegistry([claudeAgentDef]);
    const returnedDef = registry.getAgentDef('claude-code');

    returnedDef?.models.push({ id: 'lookup-model', label: 'Lookup Model' });

    expect(registry.isKnownModel('claude-code', 'lookup-model')).toBe(false);
  });

  it('protects registry state from returned provider model mutation', () => {
    const registry = createAgentRegistry([claudeAgentDef]);
    const models = registry.listProviderModels('claude-code');

    models.push({ id: 'provider-model', label: 'Provider Model' });

    expect(registry.isKnownModel('claude-code', 'provider-model')).toBe(false);
  });
  ...
  it('returns a failed connection result when probe throws', async () => {
    ...
    await expect(registry.testAgentConnection('claude-code')).resolves.toEqual({ ok: false, error: 'probe exploded' });
  });
});
