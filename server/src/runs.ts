...
export function createChatRunService(options: CreateChatRunServiceOptions): ChatRunService {
  ...
  function create(meta: ChatRunCreateMeta = {}): ChatRun {
    ...
    const run: ChatRun = {
      ...
      agentTargetId: readString(meta.agentTargetId) ?? (readString(meta.agentId) ? `local:${readString(meta.agentId)}` : null),
      provider: readString(meta.provider) ?? readString(meta.agentId),
